...
mapParser ::
  ...
  -> Parser b
--mapParser f (P a) =
--  P (\input -> case a input of 
--    ErrorResult e -> ErrorResult e
--    Result i r -> Result i (f r) ) 
mapParser f (P a) =
  P(\input -> (<$>) f (a input))
...
bindParser ::
  ...
  -> Parser a
  -> Parser b
bindParser f (P p) = -- p is the method `parse`
  P(\i -> case p i of
            ErrorResult e -> ErrorResult e
...
--   \c -> if f c then valueParser c else unexpectedCharParser c
--   \c -> iiif (unexpectedCharParser c) (valueParser c ) (f c)
   lift3 iiif unexpectedCharParser valueParser f 
